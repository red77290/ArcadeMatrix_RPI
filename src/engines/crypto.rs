...
impl Engine for CryptoEngine {
    ...
    fn render(&mut self, context: &mut EngineContext) {
        ...
        if !self.show_chart {
            if is_tate {
                // --- Vertical TATE Layout (e.g. 32x128) ---
                ...
                self.draw_plain_text(matrix, &price_str, price_x, 38, (255, 215, 0), 1.0);
            ...
            if width <= 64 {
                ...
                let (_, price_w, _) = font.get_pixel_map(&price_str, 1.0);
                let price_x = (width as i32 - price_w) / 2;
                self.draw_plain_text(matrix, &price_str, price_x, 28, (255, 215, 0), 1.0);
