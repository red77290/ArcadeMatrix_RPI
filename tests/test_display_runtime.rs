...
fn test_t01_baseline_rotation() {
    ...
    let transition = runtime.transition_session(decision, &arbiter, &mut engine_runtime);
...
fn test_t02_rotation_to_mqtt_preempt() {
    ...
    let transition = runtime.transition_session(dec2, &arbiter, &mut engine_runtime);
...
fn test_t06_mqtt_cancel_resumes_rotation() {
    ...
    let transition = runtime.transition_session(dec3, &arbiter, &mut engine_runtime);
...
fn test_t11_t12_resilient_stack_unwinding_skips_cancelled_intermediate() {
    ...
    let transition = runtime.transition_session(dec, &arbiter, &mut engine_runtime);
    assert_eq!(transition, TransitionMode::Resume);
    assert_eq!(
        runtime.active_session().source_id,
        DisplaySourceId::Rotation
