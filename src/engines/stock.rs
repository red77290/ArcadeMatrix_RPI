...
impl StockEngine {
    ...
    fn render_quote(
        ...
        draw_stock_icon(matrix, symbol, icon_x, icon_y, 16, cached_img);

        if m_w < 48 {
            draw_glcd_text(matrix, symbol, 18, 2, (255, 255, 255));
            draw_glcd_text(matrix, price_str, 2, 12, (0, 220, 255));
            ...
            draw_glcd_text(matrix, price_str, price_x, 4, (0, 220, 255));
    ...
    fn render_chart(
        ...
        draw_glcd_text(matrix, price_str, price_x, 1, (0, 220, 255));
    ...
    fn render_unified_vertical(
        ...
        if m_w >= 48 {
            draw_stock_icon(matrix, symbol, 2, 2, 16, cached_img);
            ...
            draw_glcd_text(matrix, price_str, 20, 10, (0, 220, 255));
            ...
            // Narrow Tate 32px
            draw_stock_icon(matrix, symbol, 1, 1, 8, cached_img);

            draw_glcd_text(matrix, symbol, 11, 2, (255, 255, 255));
            draw_glcd_text(matrix, price_str, 1, 11, (0, 220, 255));
    ...
    fn render_unified_wide(
        ...
        draw_stock_icon(matrix, symbol, 4, 4, 16, cached_img);

        draw_glcd_text(matrix, symbol, 24, 4, (255, 255, 255));
        draw_glcd_text(matrix, tf_label, 24, 13, (140, 140, 140));
        draw_glcd_text(matrix, price_str, 4, 24, (0, 220, 255));
    ...
    fn render_fullscreen_quote(
        ...
        if m_w <= 64 {
            ...
            draw_stock_icon(matrix, symbol, icon_x, icon_y, icon_size, cached_img);
            ...
            if use_big_price {
                ...
                draw_glcd_text_scaled(matrix, price_str, px, price_y, 2, (0, 220, 255));
                price_y += 16 + 4;
            } else {
                let px = ((m_w - price_len * 6) / 2).max(0);
                draw_glcd_text(matrix, price_str, px, price_y, (0, 220, 255));
                price_y += 8 + 4;
            }
            ...
            draw_stock_icon(matrix, symbol, icon_x, icon_y, 32, cached_img);
            ...
            draw_glcd_text_scaled(matrix, price_str, text_x, start_y + 19, 2, (0, 220, 255));
...
fn draw_stock_icon(
    ...
    if let Some(img) = cached_img {
        ...
        let color = crate::engines::icons::get_stock_color(symbol);
...
impl Engine for StockEngine {
    ...
    fn render(&mut self, context: &mut EngineContext) {
        ...
        } else if self.current_page == StockPage::Info {
            self.render_quote(
                matrix,
                &symbol,
                &price_str,
...
mod tests {
    ...
    #[test]
    fn test_stock_price_formatting() {
        let engine = StockEngine::new(64, 32);
        assert_eq!(engine.format_price(false, 0.0), "Loading...");
        assert_eq!(engine.format_price(true, 1250.75), "$1251");
        assert_eq!(engine.format_price(true, 185.34), "$185.34");
        assert_eq!(engine.format_price(true, 0.5), "$0.50");
    }

    #[test]
    fn test_stock_render_horizontal_64x32() {
        let mut engine = StockEngine::new(64, 32);
        engine.symbols = vec!["AAPL".to_string()];
        engine.cache.insert(
            "AAPL".to_string(),
            CachedQuote {
                price: 220.5,
        ...
        engine.current_page = StockPage::Chart;
        engine.render(&mut context);
    }

    #[test]
    fn test_stock_render_vertical_tate_32x64() {
        let mut engine = StockEngine::new(32, 64);
        engine.symbols = vec!["NVDA".to_string()];
        engine.cache.insert(
            "NVDA".to_string(),
            CachedQuote {
                price: 130.0,
    ...
    #[test]
    fn test_stock_render_square_and_wide() {
        let mut engine = StockEngine::new(64, 64);
        engine.symbols = vec!["TSLA".to_string()];
        engine.cache.insert(
            "TSLA".to_string(),
            CachedQuote {
                price: 240.2,
